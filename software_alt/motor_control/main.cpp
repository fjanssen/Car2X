...
bool waitForWelcome()
{
	...
	while(true)
	{
		// Try 5 times then return with false
		if(uiTries >= 5)
		{
			*pLED = *pLED | 0x01;
			return false;
	...
	// Answer the WelcomeMessage
	*pLED = *pLED & 0xFE;
	((CWelcomeMessage*) pMessage)->answerMessage(FIRMWARE_VERSION, COMPONENT_TYPE, COMPONENT_ID, uiAvailableOperations);

	// Send (if possible) the answer of the WelcomeMessage
	...
	// If everything was send return true
	return true;
}
