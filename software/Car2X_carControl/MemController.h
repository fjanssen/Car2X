...
public:
	...
	template<typename T> ErrCode pushElement(MemController::AreaId areaId, T &element);
	template<typename T> T getLastElement(MemController::AreaId areaId);
	template<typename T> T getElement(MemController::AreaId areaId, uint32_t negIndex);
	template<typename T> ErrCode getAllElements(
