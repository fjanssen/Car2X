...
void CCarProtocol::parsePacket(alt_u8 *pPacket, int iLength)
{
	...
	if(uiPayloadLength > 0)
	{
		...
		while(uiPayloadOffset < uiPayloadLength && m_uiMessageCount < 8)
		{
			...
			case 0x10: // TODO
			{
				// TODO: check why the velocity message doesnt build
				m_pMessages[m_uiMessageCount] = new CADCValuesMessage(pPacket+uiStartIdx+uiPayloadOffset, uiPayloadLength-uiPayloadOffset);//CVelocityMessage(pPacket+uiStartIdx+uiPayloadOffset, uiPayloadLength-uiPayloadOffset);
				uiPayloadOffset += 12;
				m_uiMessageCount++;
				break;
			}
